...
class Attention(nn.Module):
    ...
    def forward(self, x, prompt: nn.Module =None):
        ...
        # batch_size * num_heads, num_patch, dim
        ...
        if prompt is not None:
            pk, pv = prompt   # batch_size, num_patch, dim
            pk = pk.reshape(B, -1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).flatten(0, 1)
            pv = pv.reshape(B, -1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).flatten(0, 1)
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.attn = Attention(d_model, n_head, qkv_bias=True, attn_mask=attn_mask)
    ...
    def forward(self, x: torch.Tensor, prompt: nn.Module =None):
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, task_id: int =None):
        super().__init__()
        self.width = width
        self.layers = layers
    ...
    def forward(self, x: torch.Tensor, prompt: nn.Module =None, q: torch.Tensor = None, train: bool = False):
        prompt_loss = torch.zeros((1,), requires_grad=True).cuda()
        for i, blk in enumerate(self.resblocks):
            if prompt is not None:
                if train:
                    p_list, loss, x = prompt.forward(q, i, x, train=True, task_id=self.task_id)
                    prompt_loss += loss
                else:
                    p_list, _, x = prompt.forward(q, i, x, train=False, task_id=self.task_id)
            else:
                p_list = None
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, task_id: int=None):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        self.transformer = Transformer(width, layers, heads, attn_mask=None, task_id=task_id)

        self.ln_post = LayerNorm(width)
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))

    def forward(self, x: torch.Tensor, prompt: bool =False, q: torch.Tensor = None, train: bool = False):
        ...
        if prompt:
            ...
            prompt = None
        x, prompt_loss = self.transformer(x, prompt, q, train)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, prompt=False, q=None, train=False):
        return self.visual(image.type(self.dtype), prompt, q, train)
...
def ViT_B_16(cfg):
    ...
    model_args = dict(
        ...
        transformer_layers = 12,  init_checkpoint = cfg.MODEL.VLM.LOAD,
        num_tasks = num_tasks
    )
