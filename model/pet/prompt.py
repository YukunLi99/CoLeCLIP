...
class Prompt(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        # x: num_patches, batch_size, dim
        # timm need: batch_size, num_patches, dim
        ...
        if self.position > 0:
            ...
            return torch.cat([x1, tokens, x2], dim=1).permute(1, 0, 2)
        return torch.cat([tokens, x], dim=1).permute(1, 0, 2)
